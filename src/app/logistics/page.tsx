...
export default function LogisticsHub() {
    ...
    if (!isLookingUp && directTrackId) {
      if (directTask) {
        ...
        toast({ 
          ...
          description: "This manifest ID does not exist or you lack authorization.", 
          variant: "destructive" 
        });
        setDirectTrackId("");
      }
  ...
  }, [shipments, searchQuery, statusFilter, typeFilter]);
  ...
  const handleRequestShipping = () => {
      ...
      statusHistory: [
        {
          ...
          note: 'Shipping request submitted by customer.',
          operator: 'System'
        }
      ]
    };
    ...
    addDocumentNonBlocking(collection(firestore, 'deliveryTasks'), shipmentData).then(() => {
      toast({ title: "Fulfillment Authorized!", description: "Awaiting Agent verification." });
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex items-center justify-between px-1">
              <p className="text-[9px] font-black uppercase text-muted-foreground tracking-widest">
                Showing {filteredShipments.length} {filteredShipments.length === 1 ? 'Task' : 'Tasks'}
              </p>
              {searchQuery && (
                    ...
                    <div className="flex gap-3 items-center">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant="outline" className="text-[7px] font-black uppercase h-4 px-1.5 border-primary/20">{shipment.status}</Badge>
                          <Badge className={cn(
                      ...
                      <Popover>
                        <PopoverTrigger asChild>
                          <Button 
                            variant="ghost" 
                            size="icon" 
                            className="h-8 w-8 rounded-full opacity-30 hover:opacity-100 transition-opacity bg-muted/30" 
        ...
        <TabsContent value="new-shipment">
          <Card className="border-none shadow-xl rounded-[2.5rem] overflow-hidden bg-white">
            <CardHeader className="p-8 pb-4">
              <CardTitle className="text-xl font-black uppercase tracking-tighter">Shipment Protocol</CardTitle>
            ...
            <div className="flex flex-col h-full">
              <SheetHeader className="p-10 bg-muted/20 border-b">
                <div className="flex justify-between items-start mb-4">
                  <SheetTitle className="text-3xl font-black uppercase tracking-tighter">{selectedShipment.id.slice(0, 8)}</SheetTitle>
                ...
                <SheetDescription className="text-[10px] font-black uppercase tracking-[0.3em] text-primary">{selectedShipment.serviceType}</SheetDescription>
              </SheetHeader>
              <div className="flex-1 p-10 space-y-10">
                <div className="bg-muted/30 p-8 rounded-[2.5rem] border-4 border-dashed space-y-6 shadow-inner">
                  <div className="space-y-1.5">
                    ...
                    <p className="text-sm font-bold leading-tight">{selectedShipment.destination}</p>
                  </div>
                  <div className="space-y-1.5">
                    <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Origin Point</p>
                    <p className="text-sm font-bold leading-tight">{selectedShipment.origin || "Processing Node"}</p>
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <Badge className={cn(
                      ...
                      selectedShipment.priority === 'High' ? "bg-red-600 text-white" : 
                      selectedShipment.priority === 'Low' ? "bg-blue-400 text-white" : "bg-gray-500 text-white"
                    )}>
                      {selectedShipment.priority || 'Medium'}
                    </Badge>
                  </div>
                  {selectedShipment.packageImageUrl && (
                    <div className="space-y-1.5 pt-2">
                       ...
                       <div className="relative aspect-video rounded-2xl overflow-hidden border-2 bg-muted/50">
                         <img src={selectedShipment.packageImageUrl} alt="Package" className="object-cover w-full h-full" />
                       </div>
                    </div>
                  )}
                  <div className="space-y-1.5">
                    <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Status Phase</p>
                    <Badge className="bg-primary text-white border-none font-black text-xs h-7 uppercase px-4 rounded-xl">{selectedShipment.status}</Badge>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="relative pl-6 space-y-8 before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-[2px] before:bg-muted-foreground/10 before:rounded-full">
                    {selectedShipment.statusHistory && selectedShipment.statusHistory.length > 0 ? (
                      [...selectedShipment.statusHistory].reverse().map((entry: any, idx: number) => (
                      ...
                      <div className="relative">
                        ...
                        <div className="space-y-1">
                          ...
                          <p className="text-[9px] text-muted-foreground font-black uppercase opacity-60">
                            {new Date(selectedShipment.createdAt).toLocaleString([], { dateStyle: 'short', timeStyle: 'short' })}
                          </p>
                ...
                <div className="space-y-6">
                  <div className="flex justify-between items-center border-b pb-4">
                    ...
                    <span className="text-xs font-bold">{new Date(selectedShipment.createdAt).toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between items-center border-b pb-4">
                    ...
                    <span className="text-xs font-bold text-green-600">
                      {selectedShipment.status === 'Delivered' 
                        ? 'Delivered' 
                        : new Date(new Date(selectedShipment.createdAt).getTime() + 48 * 60 * 60 * 1000).toLocaleDateString() + ' (Approx.)'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center border-b pb-4">
                    <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Partner Recipient</span>
                    <span className="text-xs font-bold uppercase">{selectedShipment.receiverName || "Authorized Unit"}</span>
                  </div>
                  <div className="flex justify-between items-center border-b pb-4">
                    <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Settlement Value</span>
                    <span className="text-xl font-black text-primary">₦{(selectedShipment.totalAmount || 0).toLocaleString()}</span>
                  </div>
                </div>
              </div>
              <div className="p-10 bg-muted/10 border-t space-y-4 mt-auto no-print pb-12">
                <Button className="w-full h-16 font-black gap-3 rounded-2xl text-sm uppercase shadow-lg" variant="outline" onClick={() => triggerReceiptPrint()}>
                  <Printer className="h-5 w-5" /> Thermal Receipt
                </Button>
                <Button variant="ghost" className="w-full text-[10px] font-black uppercase tracking-[0.3em] opacity-40 hover:bg-transparent" onClick={() => setSelectedShipment(null)}>Close Manifest</Button>
              </div>
            </div>
          )}
        </SheetContent>
